...
//         const limit = 100; // Process records in chunks of 1000
//         let offset = 0;
...
const getImageSizes = (metaValue) => {
    try {
        console.log("-----------------------------------------------------------------")
        // Unserialize the meta_value from wp_postmeta
        const metadata = phpUnserialize.unserialize(metaValue);
        // console.log(metadata, "```````````````````")
        // Extract the original file path and the size you need (e.g., 'medium')
        const originalFilePath = metadata.file;
        // const mediumSize = metadata.sizes.medium;

        // // Construct the new image path for image_small
        // if (mediumSize && mediumSize.file) {
        //     const pathParts = originalFilePath.split('/');
        //     const baseDir = pathParts.slice(0, -1).join('/');  // Extract the directory (e.g., '2019/08')
        //     const mediumFile = mediumSize.file;

        //     // Combine the directory path with the resized image file path
        //     return `${baseDir}/${mediumFile}`;
        // }

        const constructImagePath = (sizeKey) => {
            const sizeData = metadata.sizes[sizeKey];
            if (sizeData && sizeData.file) {
                const pathParts = originalFilePath.split('/');
                const baseDir = pathParts.slice(0, -1).join('/'); // Extract the directory (e.g., '2019/08')
                const fileName = sizeData.file;

                // Combine the directory path with the resized image file path
                return `${baseDir}/${fileName}`;
            }
            return null;
        }; const image_small = constructImagePath('medium');
        const image_mid = constructImagePath('medium_large');
        const image_big = constructImagePath('large');

        return { image_small, image_mid, image_big };
    } catch (error) {
    ...
    try {
        ...
        while (true) {
            ...
            const postData = await Promise.all(wpPosts.map(async (post) => {
                ...
                if (metaValue) {
                    const { image_small, image_mid, image_big } = getImageSizes(metaValue);

                    // Assign values to the postData object
